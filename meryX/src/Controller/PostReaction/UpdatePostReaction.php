...
#[AsController]
class UpdatePostReaction extends AbstractController
{
    public function __invoke(EntityManagerInterface $em, Request $request, PostReactionRepository $postReactionRepository, CompanyPostRepository $companyPostRepo, UserRepository $userRepo, int $id)
    {
        $postReaction = $postReactionRepository->find($id);
        if (!$postReaction) {
            return $this->json(['message' => 'Post reaction not found'], 404);
        }
        ...
        if ($request->isMethod('PUT') || $request->isMethod('PATCH') || $request->isMethod('POST')) {
            if (isset($data['type'])) {
                $postReaction->setType($data['type']);
            }
            ...
            return $this->json($this->buildPayload($postReaction), 200);
        }
